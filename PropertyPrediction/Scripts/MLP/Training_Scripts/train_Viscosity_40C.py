...
dataset = pd.read_csv("Datasets/Viscosity_40C_Train_SMILES.csv")
test_dataset = pd.read_csv("Datasets/Viscosity_40C_Test_SMILES.csv")
...
def manual_grid_search(model_params_grid, training_params_grid, X, y, data_sizes, k=5):
    ...
    for data_size in data_sizes:
        ...
        for model_v, train_v in product(product(*model_values), product(*train_values)):
            ...
            results_df.to_csv('model_performance_Viscosity_40C.csv', index=False)

    return best_params

# Perform grid search
best_params = manual_grid_search(param_grid, training_params, X_train, y_train, data_sizes)
print(f"Best hyperparameters for Viscosity at 40°C: {best_params}")
...
# Save the best model
best_model.save(f"best_model_Viscosity_40C.h5")

# Evaluate the model on test data
y_pred = best_model.predict(X_test)
mse = mean_squared_error(y_test, y_pred)
print(f"Test MSE for Viscosity at 40°C: {mse}")
