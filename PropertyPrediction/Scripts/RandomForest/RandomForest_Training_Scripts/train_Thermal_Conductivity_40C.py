...
RDS = False
if RDS:
    descriptors_df = pd.read_csv('/rds/general/user/eeo21/home/HIGH_THROUGHPUT_STUDIES/MLForTribology/Datasets/Thermal_Conductivity_40C_Descriptors.csv')
...
def manual_grid_search(param_grid, X, y, k=5):
    ...
    for v in product(*values):
        ...
        results_df.to_csv('random_forest_Thermal_Conductivity_40C_training_results.csv', mode='a', header=not os.path.isfile('random_forest_Thermal_Conductivity_40C_training_results.csv'), index=False)
        header_written = True
...
# Save the best model
joblib.dump(best_model, 'best_random_forest_model_Thermal_Conductivity_40C.pkl')

# Load the best model
loaded_model = joblib.load('best_random_forest_model_Thermal_Conductivity_40C.pkl')
...
plt.figure()
plt.title("Learning Curve for RandomForestRegressor (Thermal Conductivity at 40°C)")
...
plt.legend(loc="best")
plt.savefig('random_forest_learning_curve_Thermal_Conductivity_40C.png')
plt.show()
