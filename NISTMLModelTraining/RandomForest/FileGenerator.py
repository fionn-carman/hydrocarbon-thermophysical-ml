...
RDS = False
if RDS:
    descriptors_df = pd.read_csv('/rds/general/user/eeo21/home/HIGH_THROUGHPUT_STUDIES/MLForTribology/Datasets/{property_name}_Descriptors.csv')
    test_df = pd.read_csv('/rds/general/user/eeo21/home/HIGH_THROUGHPUT_STUDIES/MLForTribology/Datasets/{property_name}_Test_Descriptors.csv')
else:
...
def manual_grid_search(param_grid, X, y, k=5):
    ...
    for v in product(*values):
        ...
        results_df.to_csv('random_forest_{property_name}_training_results.csv', mode='a', header=not os.path.isfile('random_forest_{property_name}_training_results.csv'), index=False)
        header_written = True
...
# Save the best model
joblib.dump(best_model, 'best_random_forest_model_{property_name}.pkl')

# Load the best model
loaded_model = joblib.load('best_random_forest_model_{property_name}.pkl')
...
plt.figure()
plt.title("Learning Curve for RandomForestRegressor ({description})")
...
plt.legend(loc="best")
plt.savefig('random_forest_learning_curve_{property_name}.png')
plt.show()
"""

# Generate individual scripts
output_dir = "RandomForest_Training_Scripts"
